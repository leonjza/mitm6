...
from scapy.all import sniff, ls, ARP, IPv6, DNS, DNSRR, Ether, IP, UDP
    ...
    def __init__(self, args):
        ...
        if args.ipv6 is None:
            try:
                ...
                print('Error: The interface {0} does not have an IPv6 address assigned. '
                      'Make sure IPv6 is activated on this interface.'.format(self.default_if))
                sys.exit(1)
        else:
...
def send_ra():
    # Send a Router Advertisement with the "managed" and "other" flag set,
    # which should cause clients to use DHCPv6 and ask us for addresses
...
def shutdownnotice():
    ...
    # print(pcdict)
    # print(arptable)
    with open('arp.cache', 'w') as arpcachefile:
        arpcachefile.write(json.dumps(arptable))
...
def main():
    ...
    parser.add_argument("-l", "--localdomain", type=str, metavar='LOCALDOMAIN',
                        help="Domain name to use as DNS search domain (default: use first DNS domain)")
    parser.add_argument("-4", "--ipv4", type=str, metavar='ADDRESS',
                        help="IPv4 address to send packets from (default: autodetect)")
    parser.add_argument("-6", "--ipv6", type=str, metavar='ADDRESS',
    ...
    filtergroup.add_argument("-d", "--domain", action='append', default=[], metavar='DOMAIN',
                             help="Domain name to filter DNS queries on "
                                  "(Whitelist principle, multiple can be specified.)")
    filtergroup.add_argument("-b", "--blacklist", action='append', default=[], metavar='DOMAIN',
                             help="Domain name to filter DNS queries on "
                                  "(Blacklist principle, multiple can be specified.)")
    filtergroup.add_argument("-hw", "--host-whitelist", action='append', default=[], metavar='DOMAIN',
                             help="Hostname (FQDN) to filter DHCPv6 queries on "
                                  "(Whitelist principle, multiple can be specified.)")
    filtergroup.add_argument("-hb", "--host-blacklist", action='append', default=[], metavar='DOMAIN',
                             help="Hostname (FQDN) to filter DHCPv6 queries on "
                                  "(Blacklist principle, multiple can be specified.)")
    filtergroup.add_argument("--ignore-nofqnd", action='store_true',
                             help="Ignore DHCPv6 queries that do not contain the Fully "
